...
## Message authorship and signature

When drafting direct messages on the user's behalf, make the assistant role explicit unless the user asks for a different framing.

Default rule:

- draft messages as written by Karol's Assistant, not as Karol personally
- when appropriate, say explicitly that the note comes from Karol's Assistant
- sign outbound drafts as `Karol's Assistant` unless the user asks for another signature or for no signature

Do not silently switch between first-person voice as Karol and assistant voice. If the user wants the message to sound like Karol personally, follow that instruction explicitly.
...
node ./scripts/send-connection-invite.mjs "https://www.linkedin.com/in/some-profile/" "..."
```

For already-approved mutating actions, pass `--yes` to skip the CLI confirmation prompt.
...
4. Ask for approval.
5. Execute with `send`. If the exact message is already approved in the current conversation, prefer `--yes`.
...
3. Ask for approval.
4. Execute with `comment` when a direct post URL is available. If the exact comment is already approved in the current conversation, prefer `--yes`.
...
5. Ask for approval.
6. Execute with `send`. If the exact message is already approved in the current conversation, prefer `--yes`.
...
4. Ask for approval.
5. Execute with `comment` when the item has a real post URL. If the exact comment is already approved in the current conversation, prefer `--yes`.
...
- Never send a connection invite without explicit approval in the current conversation.
- Treat networking actions as manual-review unless there is an explicit executor command for them.
